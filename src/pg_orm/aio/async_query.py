...
class AsyncQuery(ABC):
    ...
    async def first(self):
        if not (result := await (await self._session.execute(self)).first()):
            return None
        return await self._build_result(result=result)

    async def scalar(self):
        await self._session.execute(self)
        if not (result := await self._session.scalar()):
            return None
        return await self._build_result(result=result)
    ...
    async def all(self):
        await self._session.execute(self)
        if not (result := await self._session.all()):
            return list()
        return [await self._build_result(result=res) for res in result]
